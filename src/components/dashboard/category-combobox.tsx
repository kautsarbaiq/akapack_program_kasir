...
// Pemilih kategori dengan pencarian (ketik untuk filter) — untuk daftar kategori yang panjang.
export function CategoryCombobox({ categories, value, onChange, placeholder = 'Pilih kategori…' }: Props) {
  ...
  const filtered = rankedSearch(categories, q, (c) => [c.name], (c) => c.name).slice(0, 80)
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-full rounded-lg border bg-background shadow-lg">
          <div className="p-2 border-b relative">
            ...
            <Input autoFocus value={q} onChange={(e) => setQ(e.target.value)} placeholder="Ketik nama kategori…" className="h-8 pl-8 text-sm" />
          </div>
          <div className="max-h-56 overflow-y-auto py-1">
            {filtered.map((c) => (
              <button key={c.id} type="button" onClick={() => { onChange(c.id); setOpen(false); setQ('') }}
                ...
                {c.id === value && <Check size={14} className="text-primary shrink-0" />}
              </button>
            ))}
            {filtered.length === 0 && <p className="text-center text-xs text-muted-foreground py-3">Tidak ada kategori</p>}
          </div>
        </div>
      )}
    </div>
